...
class Metadetect(dict):
    ...
    def _measure(
        ...
        t0 = time.time()
        all_res = []
        for fitter, fwhm_reg, is_wavg, symm, coadd, wavg in zip(
            self._fitters, self._fwhm_regs,
            self._fitter_is_wavg, self._fitter_symmetrize,
            self._fitter_coadd, self._fitter_wavg,
        ):
            ft0 = time.time()
            if is_wavg:
