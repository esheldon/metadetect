...
class Sim(dict):
    ...
    def _get_dxdy(self):
        ...
        pos = np.array([
            self._rng.uniform(
                low=self['dims'][0] * yranges[self._pos_index][0],
                high=self['dims'][0] * yranges[self._pos_index][1]),
            self._rng.uniform(
                low=self['dims'][1] * xranges[self._pos_index][0],
                high=self['dims'][1] * xranges[self._pos_index][1])])
        # advance and wrap around
        self._pos_index += 1
        self._pos_index = self._pos_index % 4
