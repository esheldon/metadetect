...
if DO_MDET:
    def _run_sim_mdet(seed, pifffiles):
        ...
        mbobs = Sim(rng, pifffiles, config={'g1': 0.02}).get_mbobs()
...
inds = np.random.RandomState(seed).choice(
    len(piffs),
    size=n_sims * 100,
    replace=True).reshape(n_sims, 100)

sims = [joblib.delayed(_func)(s, [piffs[i] for i in pi])
        for pi, s in zip(inds, seeds)]
